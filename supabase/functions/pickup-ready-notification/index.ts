  ...
  const requestBody = {
    message: {
      ...
      kakaoOptions: {
        ...
        templateId: 'KA01TP260516143632464sZ1UJThO9wj',
        variables: {
          '#{닉네임}': nickname || '고객',
          '#{예약번호}': reservationId || '-',
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`Sending pickup ready KakaoTalk to ${member.phone}`)
    ...
    if (kakaoResult.statusCode !== '2000') {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to send pickup ready message', details: kakaoResult }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }

    console.log('Pickup ready notification sent successfully')
